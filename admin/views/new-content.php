...
<div class="wrap">
  ...
  <?php if (!$contentExists || $this->current_user_can_edit($this->content)): ?>
    ...
    <form id="h5p-data-report-settings-form" method="post" enctype="multipart/form-data">
      <input type="hidden" name="lifelabs-column-map-only" value="true" />
    <div class="postbox h5p-sidebar">
        <div role="button" class="h5p-toggle" tabindex="0" aria-expanded="true" aria-label="<?php esc_html_e('Toggle panel', $this->plugin_slug);?>"></div>
        <h2><?php esc_html_e('Data Report Column-Value Map', $this->plugin_slug);?></h2>
        <div class="h5p-panel">
          <p class="howto"><?php esc_html_e('Column-Name Remap, separated by comma. Leave the textbox blank for no remap values.', $this->plugin_slug);?></p>
          <textarea rows="5" name="h5p-column-name-remap" class="h5p-column-name-remap"><?php if ($this->content['journey_user_data_column_map'] !== null): print esc_html($this->content['journey_user_data_column_map']);?>
										    <?php else: ?>
      <?php print esc_html(
'{
  "<h5p-id-number>_start_time" : "Time Started",
  "<h5p-id-number>_total_time_spent" : "Time Until Completion",
  "<h5p-id-number>_answers_0" : "Question One",
  "<h5p-id-number>_scores_0_fullMark" : "Sobriety - Max Possible Score",
  "<h5p-id-number>_scores_0_value" : "Sobriety - Score",
  ...
}           '
);?>
    <?php endif;?>
    </textarea>
          <p class="howto"><?php esc_html_e('Row-Value Remap, separated by comma. Leave the textbox blank for no remap values.', $this->plugin_slug);?></p>
          <textarea rows="5" name="h5p-row-value-remap" class="h5p-row-value-remap"><?php if ($this->content['journey_user_data_row_value_map'] !== null): print esc_html($this->content['journey_user_data_row_value_map']);?>
									  <?php else: ?>
    <?php print esc_html(
'{
  "<h5p-id-number>_answers_0" : [[0, "Strongly Disagree"], [1, "Disagree"], [2, "Neutral"], [3, "Agree"]],
  "<h5p-id-number>_answers_1" : [[0, "First Answer"], [1, "Second Answer"], [2, "Third Answer"]],
    ...
}      '
);?>
    <?php endif;?>
  </textarea>
	  <input type="submit" name="submit-data-map-form-button" value="<?php esc_html_e('Update Map')?>" class="button button-primary button-large"/>
        </div>
      </div>
    </form>
    <?php endif;?>
